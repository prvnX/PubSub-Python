...
def startClient(server_ip, server_port, role, topic):
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

    try:
        client_socket.connect((server_ip, server_port))

        # Send role and topic to establish the connection
        client_socket.send(f"{role} {topic}".encode("utf-8"))
        ...
        print(f"[CLIENT] Connected to server at {server_ip}:{server_port} as {role} on topic {topic}.")
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        ...
        print("Example: python client.py 127.0.0.1 5000 PUBLISHER MyTopic")
        sys.exit()
    ...
    startClient(server_ip, server_port, role, topic)